...
## M7 Robustness And Negative Controls
...
The M7 workflow reuses the saved M2 grouped folds and M3 XROCKET feature
artifacts. It does not rerun or refit the XROCKET encoder, so the analysis
tests the saved padded representation, the classifier layer, and the existing
explanation aggregations. Full encoder seed sensitivity is left as future work
unless a control indicates leakage or severe instability.

The label-shuffle negative control permutes only the training labels inside
each participant-held-out fold, using seeds `7`, `13`, `29`, `42`, and `101`.
The test labels remain true labels. The strongest saved label-shuffle result
was logistic regression on XROCKET features, with mean macro F1 `0.1605` and
maximum fold/seed macro F1 `0.3151`, below the configured leakage thresholds
of `0.25` mean and `0.40` max.

Metadata controls use `original_length`, padding fraction, and `arm_code` on
the same grouped folds. The strongest metadata-only result was metadata random
forest with mean macro F1 `0.2158`, below the configured confound threshold
of `0.30`. Mean original sequence length differs by up to `6.65` samples
across movement labels, so length remains documented as a weak confound risk,
but it is not sufficient to explain the main model performance.

Seed stability retrains only the classifiers on saved XROCKET features across
the same five seeds. The primary random forest has mean macro F1 `0.8842`
with standard deviation `0.0675` across seed/fold cases, close to the original
M3 result. The top sensor family, axis, dilation, and temporal-scale bin remain
stable as gravity, `z`, dilation `6`, and long scale. Channel-level top ranks
are less stable, so channel-specific claims should be weaker than family,
axis, or temporal-scale claims.

Arm-stratified axis ranking is supported because each arm and fold subset has
complete class coverage. The right-arm code `d` ranks `x` highest on average,
while the left-arm code `i` ranks `z` highest. The pooled `z`-axis conclusion
therefore needs an arm-orientation caveat rather than being treated as a
universal anatomical-axis result.

M7 also reviews the existing M3 confusion matrices. Classes `2` and `3` are
the weakest movement-specific cases, with recalls `0.7500` and `0.7750`.
Their main confusion is between labels `2` and `3`, so final claims should
name these movement-specific weaknesses instead of relying only on aggregate
macro F1.
